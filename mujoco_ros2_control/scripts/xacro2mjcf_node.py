...
class Xacro2Mjcf(Node):
    ...
    def __init__(self):
        ...
        for robot_description in robot_descriptions:
            name = str(uuid.uuid4())
            with open('/tmp/tmp_' + name + '.urdf', 'w') as f:
                f.write(str(robot_description))
            input_files.append('/tmp/tmp_' + name + '.urdf')
        ...
        for i, input_file in enumerate(input_files):
            ...
            if filename.split('.')[-1] == 'xacro' or filename.split('.')[-1] == 'urdf':
                ...
                if filename.split('.')[-1] == 'xacro':
                    os.system('xacro ' + filename + ' > /tmp/tmp_' + name + '.urdf')
                    os.system(compile_executable + ' /tmp/tmp_' + name + '.urdf /tmp/tmp_' + name + '.xml')

                    # Get the actuators element from the urdf (in robot/mujoco/actuators
                    in_tree = ET.parse('/tmp/tmp_' + name + '.urdf')
                else:
                    os.system(compile_executable + ' ' + filename + ' /tmp/tmp_' + name + '.xml')
                    # Get the actuators element from the urdf (in robot/mujoco/actuators
                    in_tree = ET.parse(filename)

                in_root = in_tree.getroot()
                # Insert the elements in the mjcf file tree
                out_tree = ET.parse('/tmp/tmp_' + name + '.xml')
