...
LidarImpl::LidarImpl(rclcpp::Node::SharedPtr &node,
                             mjModel_ *model,
                             mjData_ *data,
                             std::mutex *sim_mutex,
                             int site_id,
    ...
    if (!snapshot_data_) {
        throw std::runtime_error("LidarImpl: mj_makeData failed");
    }
    ...
    if (slice_render_h_fov_ >= M_PI || v_fov_ >= M_PI) {
        throw std::runtime_error(
            "LidarImpl '" + name_ + "': render FOV >= pi rad, "
    ...
    RCLCPP_INFO(nh_->get_logger(),
                "LidarImpl '%s': total_h_fov=%.1f deg in %d slices of %.1f deg, "
    ...
    if (egl_display_ == EGL_NO_DISPLAY) {
        throw std::runtime_error("LidarImpl: eglGetDisplay failed");
    }
    EGLint egl_major, egl_minor;
    if (!eglInitialize(egl_display_, &egl_major, &egl_minor)) {
        throw std::runtime_error("LidarImpl: eglInitialize failed");
    }
    if (!eglBindAPI(EGL_OPENGL_API)) {
        throw std::runtime_error("LidarImpl: eglBindAPI(EGL_OPENGL_API) failed");
    }
    ...
    if (!eglChooseConfig(egl_display_, config_attribs, &egl_config, 1, &num_configs) || num_configs < 1) {
        throw std::runtime_error("LidarImpl: eglChooseConfig failed");
    }
    egl_context_ = eglCreateContext(egl_display_, egl_config, EGL_NO_CONTEXT, nullptr);
    if (egl_context_ == EGL_NO_CONTEXT) {
        throw std::runtime_error("LidarImpl: eglCreateContext failed");
    }
    ...
    if (!eglMakeCurrent(egl_display_, EGL_NO_SURFACE, EGL_NO_SURFACE, egl_context_)) {
        throw std::runtime_error("LidarImpl: eglMakeCurrent failed");
    }
    ...
    RCLCPP_INFO(nh_->get_logger(),
                "LidarImpl '%s' on site_id=%d ready: %s, %d beams%s @ %.1f Hz, "
...
void LidarImpl::release_resources() noexcept {
...
bool LidarImpl::sample_beam(size_t beam_index, double &range,
...
void LidarImpl::publish_scan(const rclcpp::Time &stamp) {
...
void LidarImpl::publish_cloud(const rclcpp::Time &stamp) {
